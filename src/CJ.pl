...
# Commented out Firebase-related imports
# use Firebase; 
use Ouch;
use File::chdir;
...
# Commented out Firebase checks for sync and info commands
...
my $spec = <<'EOSPEC';
     ...
     init 	    					  initiates CJ installation [nocase]
              							{defer{CJ::init}}
     install        [-f[<orce>]] <app:/\S+/> <cluster:/\S*/>	install app on a remote machine ('cj avail app' lists choices; e.g. miniconda, container)
